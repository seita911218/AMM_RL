...
def tune_dqn_with_ray(
    ...
    *,
    ...
    print(f"[Tune] BASE STATE_DIM={STATE_DIM}, ACTION_DIM={ACTION_DIM}, IF_DISCRETE={IF_DISCRETE}, T_BASE={T_BASE}")
    ...
    def install_tune_logger():
        ...
        def _csv_and_report(self, actor=None, steps=0, exp_r=None, logging_tuple=None):
            ...
            # report (中途先回報 latest；final_equity 會在最後覆蓋)
            tune_report({
    ...
    def make_trainable():
        def trainable_ray(hp: dict):
            ...
            cfg.horizon_len     = int(hp["horizon_len"])   # 可與 episode_len 不同
    ...
    if param_space is None:
        ...
        param_space = {
            ...
            "eps":        _t.uniform(0.30, 0.80),               # ↑ 探索
            "buffer":     _t.qlograndint(int(2e5), int(1e6), int(1e4)),  # ↑ 多元樣本
...
def tune_ppo_with_ray(
    ...
    *,
    ...
    FEAT = int(getattr(train_data, "shape", (0, 0))[1]) if hasattr(train_data, "shape") else 0
    STATE_DIM  = int(WIN * FEAT) if WIN > 1 else int(FEAT)
    ACTION_DIM = int(TRAIN_ENV_KW.get("action_dim", 5))
    IF_DISCRETE = True
    T_BASE = int(min(int(TRAIN_ENV_KW["max_steps"]), int(train_data.shape[0] - 1)))
    print(f"[Tune] (base) STATE_DIM={STATE_DIM}, ACTION_DIM={ACTION_DIM}, IF_DISCRETE={IF_DISCRETE}, T_BASE={T_BASE}")
    ...
    if param_space is None:
        ...
        param_space = {
            ...
            "batch_size":   _t.choice([1024, 2048, 4096]),     # ↓ 更 noisy
            ...
            "clip_ratio":   _t.uniform(0.15, 0.35),            # ↑ 允許較大更新
            "entropy_coef": _t.loguniform(1e-3, 5e-2),         # ↑ 探索
            "vf_coef":      _t.uniform(0.3, 0.7),
            "passes":       _t.choice([1, 2, 4, 8]),           # ↓ 避免過度磨平
            "seed":         _t.randint(0, 1_000_000),
        }
